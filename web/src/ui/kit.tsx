...
export function Avatar({ src, name, size = 32, className = '' }: { src?: string | null; name?: string; size?: number; className?: string }) {
  ...
  if (src) return <img src={src} style={s} className={`rounded-full object-cover shrink-0 ${className}`} />;
  return (
...
export function Header() {
  ...
  return (
    <div className="flex items-center gap-2.5 px-4 pt-3 pb-2">
      <Avatar src={me.photo} name={me.name} size={36} className="glow-pulse" />
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 overflow-x-auto no-scrollbar">
      <div className="shrink-0 flex items-center gap-1.5 text-[11px] font-extrabold text-lime">
        <span className="w-2 h-2 rounded-full bg-lime dot-pulse" /> LIVE {online > 0 && <span className="text-white/40">· {online}</span>}
      </div>
      {live.length === 0 && <div className="text-xs text-white/30">Здесь появятся крупные выигрыши</div>}
      {live.map((w, i) => (
        <div key={w.at + '' + i} className="row-in shrink-0 flex items-center gap-1.5 bg-moss rounded-full pl-1 pr-3 py-1">
      ...
      <div className="card grid grid-cols-4 p-1.5 max-w-md mx-auto">
        {items.map(([k, Icon, label]) => (
          <button key={k} onClick={() => go(k)} className={`flex flex-col items-center gap-1 py-2 rounded-2xl transition ${screen === k ? 'bg-lime/12 text-lime' : 'text-white/45'}`}>
            <span className={screen === k ? 'nav-pop' : ''} key={screen === k ? 'on' : 'off'}><Icon size={k === 'bitkong' ? 21 : 20} /></span>
